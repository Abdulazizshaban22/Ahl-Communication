...
export default function Chat(){
  ...
  async function exportBackup(){
    ...
    const pass = prompt('كلمة المرور للنسخة الاحتياطية:') || ''
  ...
  return (
    <main style={{fontFamily:'system-ui',padding:16,maxWidth:720,margin:'0 auto'}}>
      <header style={{display:'flex',gap:8,alignItems:'center',marginBottom:8}}>
        <h1 style={{margin:0}}>Ahla Chat — v3.2</h1>
        <span style={{fontSize:12,opacity:0.7}}>Push: {pushOK? 'ON':'OFF'}</span>
        <nav style={{marginInlineStart:'auto',display:'flex',gap:8}}>
          <a href="/chat/moments">Moments</a>
          <a href="/chat/voice">Voice Lab</a>
          <a href="/chat/verify">Verify</a>
          <a href="/chat/backup">Backup</a>
        </nav>
      </header>
      ...
      <div style={{display:'flex',gap:8,marginTop:8}}>
        ...
        <button onClick={exportBackup} style={{padding:'12px 16px',borderRadius:12}}>تصدير نسخة</button>
      </div>
    </main>
  )
}
