...
app = FastAPI(title="Ahla Chat API v3.2", version="3.2.0")

MSG_COUNTER = Counter("ahla_messages_total","Total messages posted", ["room"])
ATTACH_COUNTER = Counter("ahla_attachments_total","Total attachments uploaded", ["room"])
TONE_COUNTER = Counter("ahla_tone_requests_total","Tone API requests", [])
...
async def upload(room:str, file: UploadFile = File(...)):
    ...
    ATTACH_COUNTER.labels(room=room).inc()
...
@app.get("/push/vapidPublicKey")
def vapid_public(): return {"key": VAPID_PUBLIC}

@app.post("/push/subscribe")
def subscribe(user: str = Body(...), sub: dict = Body(...)):
    if r: r.sadd(f"subs:{user}", json.dumps(sub))
    return {"ok": True}
