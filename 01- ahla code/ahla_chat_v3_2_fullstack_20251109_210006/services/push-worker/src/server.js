...
if(vapidPublic && vapidPrivate){
  ...
  console.warn('[push-worker] VAPID keys not set; push disabled')
}

app.post('/sendTo', async (req,res)=>{
  const user = req.body.user || 'me'
  const body = req.body.body || 'New message'
  const room = req.body.room || 'personal'
  const subs = await redis.smembers(`subs:${user}`)
  let sent=0, failed=0
  for(const s of subs){
    try{
      const sub = JSON.parse(s)
      await webpush.sendNotification(sub, JSON.stringify({ title:'Ahla', body, room, tag: room }))
      sent++
    }catch(e){ failed++ }
  }
