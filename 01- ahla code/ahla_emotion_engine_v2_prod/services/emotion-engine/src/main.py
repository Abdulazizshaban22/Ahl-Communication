...
app = FastAPI(title="Ahla Emotion Engine v2", version="2.0.0")
FastAPIInstrumentor.instrument_app(app)

# Pipelines (lazy load to speed startup)
...
def apply_rules(analysis: AnalysisOut, m: MessageIn, history: List[AnalysisOut]) -> List[str]:
    rules = CFG["rules"]
    suggests = []
    ...
    if m.context == "work" and joy >= rules["praise_suggest_thanks"]["joy_threshold"]:
        suggests.append("🌟 واضح إنك مبسوط من الشغل. تبغاني أجهّز رسالة شكر راقية؟")

    if m.ts.hour >= rules["night_overwork"]["hour_start"]:
