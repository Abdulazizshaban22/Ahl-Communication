...
export default function Meet(){
  ...
  const localRef=useRef<HTMLVideoElement>(null), remoteRef=useRef<HTMLVideoElement>(null)
  const pcRef=useRef<RTCPeerConnection|null>(null); const wsRef=useRef<WebSocket|null>(null)
  useEffect(()=>{ (async ()=>{
      const ice=await fetch('/api/meet/ice').then(r=>r.json()).catch(()=>({iceServers:[]}))
      const stream=await navigator.mediaDevices.getUserMedia({audio:true,video:true})
      if(localRef.current){ localRef.current.srcObject=stream; await localRef.current.play() }
      const pc=new RTCPeerConnection(ice); stream.getTracks().forEach(t=>pc.addTrack(t,stream))
      ...
      ws.onmessage=async ev=>{ const {type,data}=JSON.parse(ev.data)
      ...
      wsRef.current=ws; pc.onicecandidate=ev=>{ if(ev.candidate) ws.send(JSON.stringify({type:'candidate',data:ev.candidate})) }
      const offer=await pc.createOffer(); await pc.setLocalDescription(offer)
      ws.addEventListener('open', ()=> ws.send(JSON.stringify({type:'offer',data:offer})))
  })(); return ()=>{ wsRef.current?.close(); pcRef.current?.close() } },[room])
  return <main style={{fontFamily:'system-ui',padding:16}}>
    <h1>Ahla Meet</h1>
    <input value={room} onChange={e=>setRoom(e.target.value)} style={{padding:8,marginBottom:8}}/>
