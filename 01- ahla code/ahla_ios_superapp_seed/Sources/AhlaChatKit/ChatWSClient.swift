...
public final class ChatWSClient: NSObject, ObservableObject {
    ...
    public func connect(room: String, token: String? = nil) {
    ...
    private func listen() {
        task?.receive { [weak self] result in
            ...
            case .success(let msg):
                ...
                case .string(let s):
                    if let d = s.data(using: .utf8), let cm = try? JSONDecoder().decode(ChatMessage.self, from: d) {
                        DispatchQueue.main.async { self.messages.insert(cm, at: 0) }
                    }
