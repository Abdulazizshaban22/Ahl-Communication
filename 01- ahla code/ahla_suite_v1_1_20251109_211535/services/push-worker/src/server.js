...
if(publicKey && privateKey){ webpush.setVapidDetails('mailto:admin@example.com', publicKey, privateKey) }
app.post('/sendTo', async (req,res)=>{
  ...
  const subs = await redis.smembers(`subs:${user}`); let sent=0,failed=0
  for(const s of subs){ try{ await webpush.sendNotification(JSON.parse(s), payload); sent++ }catch{ failed++ } }
