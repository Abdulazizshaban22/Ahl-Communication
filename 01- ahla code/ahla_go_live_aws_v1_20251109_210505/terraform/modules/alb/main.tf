...
resource "aws_security_group" "alb" {
  ...
  ingress { from_port = 8080, to_port = 8080, protocol = "tcp", cidr_blocks = ["0.0.0.0/0"] } # test listener
...
# TEST listener for CodeDeploy (blue/green validation)
resource "aws_lb_listener" "test" {
  load_balancer_arn = aws_lb.this.arn
  port              = 8080
  protocol          = "HTTP"
  default_action { type = "fixed-response" fixed_response { content_type = "text/plain" message_body = "TEST" status_code = "200" } }
}
