...
resource "aws_ecs_service" "green" {
  ...
  network_configuration { subnets = var.private_subnets, security_groups = [ aws_security_group.tasks.id ] }
  load_balancer { target_group_arn = aws_lb_target_group.green[each.key].arn, container_name="${each.key}-web", container_port=each.value.port }
...
resource "aws_lb_listener_rule" "canary" {
  ...
  action {
    type = "forward"
    forward {
      target_group { arn = module["svc_${each.key}_web"].tg_arn, weight = 50 }
      target_group { arn = aws_lb_target_group.green[each.key].arn, weight = 50 }
